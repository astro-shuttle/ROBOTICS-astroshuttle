...
const int MOTOR_RIGHT_FORWARD_PIN = 5;  // right motor "forward" channel
const int MOTOR_RIGHT_REVERSE_PIN = 4;  // right motor "reverse" channel
...
const int SERVO_ANGLE_CfR = 90;  // for each servo if we want to set custom zeros
const int SERVO_ANGLE_CfL = 90;
const int SERVO_ANGLE_CbR = 90;
const int SERVO_ANGLE_CbL = 90;
...
void DdriveL() {
  steerL();
  SoftPWMSet(MOTOR_LEFT_FORWARD_PIN, MOTOR_SPEED_SLOW);
  SoftPWMSet(MOTOR_LEFT_REVERSE_PIN, 0);
  SoftPWMSet(MOTOR_RIGHT_FORWARD_PIN, MOTOR_SPEED_DEFAULT);
  SoftPWMSet(MOTOR_RIGHT_REVERSE_PIN, 0);
}
